...
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)): 
    ...
    if sns_type == SnsType.email: 
        ...
        _token_data = UserToken.from_orm(new_user).dict(exclude={'pw', 'marketing_agree'})
        token = dict(Authorization=f"Bearer {create_access_token(data=_token_data)}")
        return token 
...
async def login(sns_type: SnsType, user_info: UserRegister): 
    if sns_type == SnsType.email: 
        ...
        is_verified = verify_pw(user_info.pw, user.pw)
        if not is_verified: 
            return JSONResponse(status_code=400, content=dict(msg="NO_MATCH_USER"))
        _token_data = UserToken.from_orm(user).dict(exclude={'pw', 'marketing_agree'})
        token = dict(Authorization=f"Bearer {create_access_token(data=_token_data)}")
        return token 
    return JSONResponse(status_code=400, content=dict(msg="NOT_SUPPORTED")) 

async def is_email_exist(email: str): 
    email_exist = Users.get(email=email) 
    return True if email_exist else False

def verify_pw(src, trg): 
    return bcrypt.checkpw(src.encode("utf-8"), trg.encode("utf-8")) 
