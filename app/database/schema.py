...
class BaseMixin: 
    ...
    def filter(cls, session: Session = None, **kwargs): 
        ...
        for key, value in kwargs.items(): 
            ...
            if k_len == 1: condition.append((col == value)) 
            elif k_len == 2: 
                _cond = key[1] 
                if      _cond == 'gt':      condition.append(( col > value ))
                elif    _cond == 'gte':     condition.append(( col >= value ))
                elif    _cond == 'lt':      condition.append(( col < value ))
                elif    _cond == 'lte':     condition.append(( col <= value ))
                elif    _cond == 'in':      condition.append(( col.in_(value) ))
        obj = cls() 
        if session: 
    ...
    def update(self, auto_commit: bool = False, **kwargs): 
        ...
        if qs > 0: 
            ret = self._q.first() 
        if auto_commit: self._session.commit() 
        return ret 
...
class ApiKeys(Base, BaseMixin): 
    ...
    user_demo = Column(String(length=40), nullable=True)
